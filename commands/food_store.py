...
class FoodStoreCommand(commands.Cog):
    ...
    def build_store_text(
        ...
        return (
            "## 🍶 주막\n\n"
            f"**차림표 · {current_page + 1} / "
            f"{total_pages}**\n\n"
            f"{item_text}\n\n"
            "━━━━━━━━━━━━━━━━━━\n\n"
            "### 🛒 주문 목록\n"
            f"{cart_text}\n\n"
            "아래 메뉴에서 주문할 품목을 선택하세요."
        )
    ...
    async def checkout(
        ...
        try:
            ...
            await self.log_service.send_food_purchase_log(
                bot=self.bot,
...
class FoodStoreView(discord.ui.View):

    def __init__(
        self,
        command: FoodStoreCommand,
        user_id: int,
        current_page: int
    ):
